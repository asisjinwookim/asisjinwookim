...
class ModelConfigForm(forms.ModelForm):
    ...
    # 폼 초기화 시 (GET 요청 시) 기존 인스턴스에서 JSON 데이터를 텍스트로 변환하여 폼 필드에 채움 (편집 시)
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if (
            self.instance and self.instance.pk
        ):  # 인스턴스가 이미 DB에 저장된 객체인 경우
            # 모델의 parameters 필드(TextField)에 저장된 JSON 문자열을 그대로 폼 필드에 할당
            if (
                self.instance.parameters and self.instance.parameters != "{}"
            ):  # 빈 객체가 아닌 경우만 로드
                self.initial["parameters_json"] = (
                    self.instance.parameters
                )  # 이미 JSON 문자열이므로 그대로 할당
            # model_type 필드도 초기화
            self.initial["model_type"] = self.instance.model_type

    # 폼 유효성 검사 후, parameters_json의 데이터를 model.parameters에 맞게 가공합니다.
    def clean_parameters_json(self):
        ...
        try:
            ...
            return params_dict  # 파이썬 딕셔너리로 반환 (저장 시 다시 덤프할 것임)
    ...
    def save(self, commit=True):
        ...
        # clean_parameters_json 메서드에서 가공된 파이썬 딕셔너리를
        # 모델의 parameters 필드(TextField)에 JSON 문자열로 변환하여 할당
        instance.parameters = json.dumps(self.cleaned_data["parameters_json"])

        # forms.ChoiceField인 'model_type'에서 선택된 값을
        # 모델의 model_type 필드에 할당
